...
```{r}
d[ , y_0 := ifelse(age=='young', 10, 15) + rnorm(NROWS, mean = 0, sd = 1)]
d
```
...
```{r}
d[, treat := sample(c(1, 0))]
d[, Y:= ifelse(treat==1, y_1, y_0)]
d
...
```{r}
nrow(d[treat==1 & height=='tall'])
nrow(d[treat==0 & height=='tall'])
nrow(d[treat==1 & height=='short'])
nrow(d[treat==0 & height=='short'])
...
mean_y1        <- d[ , mean(y_1)]
mean_y1
mean_treatment_Y <- obs[treat==1, mean(Y)]
mean_treatment_Y
```
...
```{r}
mean_treatment_Y - mean_control_Y
d[, mean(tau)]
