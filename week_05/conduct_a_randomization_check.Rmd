...
d[, chisq.test(table(x3, treat))]

library(magrittr)

d[ , table(x3, treat)] %>% chisq.test
```
...
d[treat==1, block:=factor(sample(x = c("A", "B", "C" ), size = .N, replace=TRUE, prob=c(.1, .1, .8)))]
d[treat==0, block:=factor(sample(x = c("A", "B", "C" ), size = .N, replace=TRUE, prob=c(.5, .4, .1)))]
```

```{r}
test_1 <- d[, t.test(x1  ~ treat) ]
test_2 <- d[, t.test(x2  ~ treat) ]
test_3 <- d[, table(x3, treat) ] %>% chisq.test
test_4 <- d[ , table(block, treat)] %>% chisq.test()

p.adjust(test_4$p.value, method='bonferroni', 4)

short_mod <- d[ , lm(treat ~1)]
long_mod <- d[ , lm(treat ~1 + x1 + x2 + x3 + block)]
anova(short_mod, long_mod, test='F')
```
...
# estimate covariance checks inside of each of the blocks - multiple regression checks one per block
